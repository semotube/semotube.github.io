...
//  헤더 — 다이빙 / 우표 / 화폐 탭
// ═══════════════════════════════════════════════════════════
function DebugBar({ load, hobby, files, open, setOpen }) {
  const [probes, setProbes] = useState(null);
  const FNAME_RE = /^([a-z0-9]{2})_(\d{4})_(\d{5})\.([a-z0-9]+)$/i;
  const bad = (files || []).filter(f => !FNAME_RE.test(f));

  async function runProbe() {
    setProbes({ loading: true });
    const sample = (files || []).slice(0, 6);
    const results = await Promise.all(sample.map(async (fn) => {
      const url = `media/${hobby}/${fn}`;
      const t0 = performance.now();
      try {
        const res = await fetch(url, { method: 'HEAD', cache: 'no-store' });
        return { fn, url, ok: res.ok, status: res.status, ms: Math.round(performance.now() - t0) };
      } catch (e) {
        return { fn, url, ok: false, status: 'ERR', err: String(e.message || e), ms: Math.round(performance.now() - t0) };
      }
    }));
    setProbes({ results });
  }

  const hasIssue = !!load.error || bad.length > 0;

  return (
    <div className="border-b border-line bg-paper2/60">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 py-2 flex items-center gap-3 text-[11px] font-mono text-muted">
        <span className={hasIssue ? 'text-rose-700 font-semibold' : 'text-emerald-700'}>
          {hasIssue ? '⚠ 문제 감지' : '✓ OK'}
        </span>
        <span>인덱스: <span className="text-ink2">{load.source}</span> ({load.ms}ms)</span>
        <span className="hidden sm:inline">파일: <span className="text-ink2">{(files||[]).length}</span></span>
        {bad.length > 0 && <span className="text-rose-700">잘못된 파일명: {bad.length}</span>}
        <span className="ml-auto" />
        <button onClick={() => setOpen(!open)} className="px-2 py-0.5 rounded border border-line hover:bg-paper3 text-ink2">
          {open ? '디버그 닫기' : '디버그 열기'}
        </button>
      </div>
      {open && (
        <div className="max-w-6xl mx-auto px-5 sm:px-8 pb-3 text-[11.5px] font-mono leading-relaxed">
          <div className="grid sm:grid-cols-2 gap-4 mt-1">
            <div className="space-y-1">
              <div className="text-muted">현재 페이지: <span className="text-ink2">{hobby}</span></div>
              <div className="text-muted">기대 폴더: <span className="text-ink2">media/{hobby}/</span></div>
              <div className="text-muted">파일명 규칙: <span className="text-ink2">cc_yyyy_nnnnn.ext</span></div>
              <div className="text-muted">예: <span className="text-ink2">kr_2026_00001.jpg</span></div>
              {load.error && <div className="text-rose-700 break-all">index.json 오류: {load.error}</div>}
              {bad.length > 0 && (
                <div className="mt-2">
                  <div className="text-rose-700">잘못된 파일명 ({bad.length}):</div>
                  <ul className="text-ink2 list-disc pl-5">
                    {bad.slice(0, 8).map(f => <li key={f}>{f}</li>)}
                  </ul>
                </div>
              )}
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <button onClick={runProbe} className="px-2 py-0.5 rounded border border-line hover:bg-paper3 text-ink2">
                  파일 접근 테스트 (HEAD)
                </button>
                <span className="text-muted">앞 6개 파일 확인</span>
              </div>
              {probes?.loading && <div className="text-muted">테스트 중…</div>}
              {probes?.results && (
                <ul className="space-y-0.5">
                  {probes.results.map(r => (
                    <li key={r.url} className={r.ok ? 'text-emerald-700' : 'text-rose-700'}>
                      {r.ok ? '✓' : '✗'} {r.status} · {r.url} <span className="text-muted">({r.ms}ms)</span>
                    </li>
                  ))}
                </ul>
              )}
    ...
    <div className="fixed inset-0 z-50 bg-ink/95 backdrop-blur-md flex flex-col" onWheel={handleWheel}>
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 h-14 text-paper border-b border-paper/10">
        <div className="flex items-center gap-3 text-[14px]">
          ...
          <span className="text-paper/60">·</span>
          <span className="font-mono text-paper/60">#{String(item.seq).padStart(5, '0')}</span>
        </div>
...
function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <DebugBar load={load} hobby={route.page} files={files} open={debugOpen} setOpen={setDebugOpen} />
      <main>
